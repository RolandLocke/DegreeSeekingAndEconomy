...
def applicants_percent_change_vs_all():
...
# generate numerous graphs that will visualize a relationship between the percent change in applicants and the
# percent change of different economic indicators.
def applicants_percent_change_vs_percent_change_all():
    #create a list of column names to iterate over.
    columns = df1.columns

    for i in range(8, len(columns), 2):
        # assign the values
        applicants = df1['Applicants Percent Change']
        xVar = df1[columns[i]]
